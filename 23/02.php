...
$send_prev_y = 0;
for($i =  0; $i<50; $i++){
    $comp = new IntCode($input);
    array_push($comp->inputBuffer,$i);
    array_push($computers,$comp);
}
$done = false;
$idle = 0;
while(!$done){
    foreach($computers as $computer){
        $computer->run();
        array_push($computer->inputBuffer,-1);
        for($i = $computer->outputBufferPointer; $i < count($computer->outputBuffer); $i+=3){
            $idle++;
            $adress = $computer->outputBuffer[$i];
            $x = $computer->outputBuffer[$i+1];
            $y = $computer->outputBuffer[$i+2];
            if($adress == 255) {
                $nat_x = $x;
                $nat_y = $y;
            }
            foreach($computers as $comp){
                if($comp->inputBuffer[0] == $adress){
                    array_push($comp->inputBuffer,$x);
                    array_push($comp->inputBuffer,$y);
                }
            }
            $computer->outputBufferPointer = $i;
        }
    }
    if($idle > 5000){
        if($nat_y == $send_prev_y){
            echo "done: y: " . $nat_y . PHP_EOL;
            $done = true;
            break;
        }
        array_push($computers[0]->inputBuffer,$nat_x);
        array_push($computers[0]->inputBuffer,$nat_y);
        $send_prev_y = $nat_y;
        $idle = 0;
