...
// main routine: ABACBCBCAC
// A: R12L6R12
// B: L8L6L10
// C: R12L10L6R10

$main = str_split("A,B,A,C,B,C,B,C,A,C");
$a = str_split("R,12,L,6,R,12");
$b = str_split("L,8,L,6,L,10");
$c = str_split("R,12,L,10,L,6,R,10");

foreach($main as $char)
    array_push($robot->inputBuffer,ord($char));
array_push($robot->inputBuffer,10);
...
foreach($c as $char)
    array_push($robot->inputBuffer,ord($char));
array_push($robot->inputBuffer,10);
array_push($robot->inputBuffer,ord('n'));
array_push($robot->inputBuffer,10);

$robot->run();
print_r($robot->outputBuffer);
foreach($robot->outputBuffer as $char){
    echo chr($char);
}
